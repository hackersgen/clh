...
const app = new Vue({
  ...
  methods: {
    ...
    onStateChange: function(change) {
      console.log(
        `state changing from "${change.from}" to "${change.to}" but no handler is registered.`
      );
    },
    ...
    printGoldenCommands: function() {
      ...
      const halfScreen = Math.floor(consoleCanvas.conf.PLAY_CHARS_PER_LINE / 2);
      ...
      out += cmds
        .py()
        .name.padEnd(Math.floor(consoleCanvas.conf.PLAY_CHARS_PER_LINE / 2));
      out += cmds.html().name + "\n";
    ...
    printHighScores: function(leaders) {
      ...
      leaderContent.forEach(leader => {
        ...
        let score = leader.score.toString().padEnd(longestScoreLength + 1);
    ...
    writeToConsole: function() {
      this.$nextTick(() => {
        ...
        const showCursor = this.allowTyping && performance.now() % 1200 < 600;
        if (showCursor) {
          args[0] += "█";
        }
        if (this.showScore) {
    ...
    typingLoop: function() {
      let delay = this.typingTimeChar(this.displayCmd[this.displayCmd - 1]);

      if (!this.allowTyping) {
        this.displayCmd = this.cmd.substr(0, this.typingPosition);
      }
      // increment typing position but don't exceed the length of cmd
      this.typingPosition = Math.min(this.typingPosition + 1, this.cmd.length);
  ...
  mounted: function() {
    ...
    this.$nextTick(function() {
      ...
      // the mouse
      document.body.addEventListener("click", () => this.$refs.cmd.focus());
    });
  }
});
