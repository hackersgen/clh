...
THREE.Fire = function(geometry, options) {
  ...
  this.massConservation =
    options.massConservation === undefined ? false : options.massConservation;
    ...
    for (var y = startY; y < endY; y++) {
      for (var x = startX; x < endX; x++) {
        ...
        if (diffX * diffX + diffY * diffY < radius * radius) {
          ...
          if (density != null) {
            this.sourceData[stride] =
              Math.min(Math.max(density, 0.0), 1.0) * 255;
          }
          if (windX != null) {
            var wind = Math.min(Math.max(windX, -1.0), 1.0);
            wind =
              wind < 0.0
                ? Math.floor(wind * 127) + 255
                : Math.floor(wind * 127);
  ...
  this.field0 = new THREE.WebGLRenderTarget(
    textureWidth,
    textureHeight,
    parameters
  );
  ...
  this.field1 = new THREE.WebGLRenderTarget(
    textureWidth,
    textureHeight,
    parameters
  );
  ...
  this.fieldProj = new THREE.WebGLRenderTarget(
    textureWidth,
    textureHeight,
    parameters
  );
  ...
  this.fieldGeometry = new THREE.PlaneBufferGeometry(
    textureWidth,
    textureHeight
  );

  this.internalSource = new THREE.DataTexture(
    this.sourceData,
    textureWidth,
    textureHeight,
    THREE.RGBAFormat
  ...
  this.configShaders = function(dt) {
    ...
    this.diffuseMaterial.uniforms.expansion.value = Math.exp(
      this.expansion * -1.0
    );
    ...
    this.driftMaterial.uniforms.airSpeed.value =
      dt * this.airSpeed * 0.001 * textureHeight;
