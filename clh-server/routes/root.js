...
module.exports = async function (fastify, opts) {
      ...
      if (fileExists) {
        ...
        if (existingHeaders !== headers) {
          return reply.code(400).send({
            ...
            message:
              "The schema of the new data does not match the existing file",
          });
        }

        const existingData = existingLines
          .slice(1)
        ...
        const uniqueLeaders = leaders.filter((newLeader) => {
          return !existingData.some(
            (existingLeader) =>
              existingLeader.name === newLeader.name &&
              existingLeader.score === newLeader.score
          );
        });
  ...
  fastify.post(
    "/leaderboard/check-nickname-profanity",
  ...
  fastify.post("/leaderboard/entry", async function (request, reply) {
    ...
    const row = Object.values(leader)
      .map((value) => {
        if (typeof value === "string" && value.includes(",")) {
          return `"${value}"`;
        }
    ...
    if (fileExists) {
      ...
      if (existingHeaders !== headers) {
        return reply.code(400).send({
          error: "Schema mismatch",
          message:
            "The schema of the new entry does not match the existing file",
        });
      }
